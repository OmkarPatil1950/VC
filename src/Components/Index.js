...
function Index() {
  ...
  console.log(doctorId, "docotorid");
  console.log(patientId, "pateintid");
  ...
  useEffect(() => {
    if (videoEnabled || audioEnabled) {
      navigator.mediaDevices
        .getUserMedia({ video: videoEnabled, audio: audioEnabled }) //Used for video and audio permissions
        .then((stream) => {
  ...
  function handleConnect() {
    ...
    var socket = new WebSocket(
      "wss://web-rtc-server-git-techbrutal1151-dev.apps.sandbox-m2.ll9k.p1.openshiftapps.com/websocket"
    );
    ...
    stompClient.connect(
      ...
      (frame) => {
        ...
        const testServerSubscription = stompClient.subscribe(
          "/topic/testServer",
          function (test) {
